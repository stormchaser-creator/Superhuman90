...
CREATE TABLE IF NOT EXISTS habit_log (id UUID DEFAULT gen_random_uuid() PRIMARY KEY, device_id TEXT NOT NULL REFERENCES devices(device_id), user_id UUID DEFAULT auth.uid(), date DATE NOT NULL, h0 BOOLEAN DEFAULT false, h1 BOOLEAN DEFAULT false, h2 BOOLEAN DEFAULT false, h3 BOOLEAN DEFAULT false, h4 BOOLEAN DEFAULT false, h5 BOOLEAN DEFAULT false, h6 BOOLEAN DEFAULT false, h7 BOOLEAN DEFAULT false, created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now(), UNIQUE(device_id, date));
